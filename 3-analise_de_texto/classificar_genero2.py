...
ARQUIVO_STREAMERS2 = os.path.join(PASTA_ATUAL, "dados", "streamers_filtrado2.csv")
ARQUIVO_DESTINO = os.path.join(PASTA_ATUAL, "dados", "streamers_genero.csv")
ARQUIVO_CONTAGEM = os.path.join(PASTA_ATUAL, "dados", "contagem.csv")
ARQUIVO_CATEGORIAS = os.path.join(PASTA_ATUAL, "dados", "id_categorias.json")
ARQUIVO_CONTAGEM_MSGS = os.path.join(PASTA_ATUAL, "dados", "chat_contagem.csv")
...
if not os.path.exists(ARQUIVO_STREAMERS2):
    df_streamers = pd.read_csv(ARQUIVO_STREAMERS, sep="\t")
    df_contagem_msgs = pd.read_csv(ARQUIVO_CONTAGEM_MSGS, sep="\t")
    streamers_suficientes = df_contagem_msgs[df_contagem_msgs["mensagens"] >= 300]["user_name"]
    df_streamers = df_streamers[df_streamers["user_name"].isin(streamers_suficientes)]
    df_streamers = df_streamers.drop_duplicates(subset=["user_name"])
    df_streamers.to_csv(ARQUIVO_STREAMERS2, sep="\t", index=False)
    
df_streamers = pd.read_csv(ARQUIVO_STREAMERS2, sep="\t")
